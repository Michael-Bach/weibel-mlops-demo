...
compared against two learned detectors: a batch fully-convolutional
network (CNN+KF, 19\,073 parameters) and a streaming convolutional GRU
(ConvGRU+KF, 5\,694 parameters).
...
Cell-level ROC comparison at SNR\,=\,0\,dB shows that LRT and DP-TBD
(AUC 0.459) provide no improvement over CA-CFAR (AUC 0.482) for moving
targets on 10-sweep sequences---a consequence of integration loss when
target energy is spread across cells.
The CNN achieves AUC 0.784 at 0\,dB through learned temporal features,
confirming tracks at the lowest false track rate (0.79\,/\,100\,sweeps),
...
\subsection{Pipeline 4 --- Non-coherent LRT}
...
The key finding is that LRT and DP-TBD do \emph{not} improve over CA-CFAR
at cell level despite integrating all 10 sweeps.
The reason is integration loss from target motion: a target traversing
$v_r = 1.5$\,bins/sweep covers $\approx$\,15 range bins over 10 sweeps,
so no single cell accumulates more than 1--2 of the 10 returns.
The 5-bin scoring window captures some of this energy, but not enough to
recover the theoretical $+10\,\text{dB}$ stationary-target gain.

The CNN avoids this problem entirely: its input features (temporal max,
mean, std) are computed \emph{across the whole sequence per cell}, so
the max channel directly picks up the brightest return regardless of
which sweep it occurred in.
This is why a learned detector with only 19\,000 parameters achieves a
30-point AUC advantage over the optimal classical integrator.
...
\begin{figure}[h]
  ...
  \caption{Cell-level ROC curves at SNR\,=\,0, 3, and 6\,dB (400 trials per
           ...
           At SNR\,=\,0\,dB: CNN 0.784, CA-CFAR 0.482, ConvGRU 0.476,
           LRT 0.459, DP-TBD 0.459.
           The CNN's learned temporal features (max/mean/std across 10 sweeps)
           produce a 30-point AUC gain over the analytical integrators.
           LRT and DP-TBD match CA-CFAR despite accumulating all 10 sweeps,
           because moving-target energy is spread across cells, negating the
           stationary-target integration gain (see Section~\ref{sec:results_roc}).
           ConvGRU's modest cell-level AUC reflects that its hidden-state
           confidence is tuned for track confirmation, not raw discrimination;
           its operational advantage is in confirmed-track $P_d$
           (Table~\ref{tab:pd}).}
  \label{fig:roc}
\end{figure}
...
The central finding is that analytical multi-sweep integration (LRT,
DP-TBD) does not improve over single-sweep CFAR for moving targets on
10-sweep sequences: target motion spreads energy across cells, negating
the stationary-target integration gain.
Learned temporal features (CNN max/mean/std channels) circumvent this
limitation and achieve a 30-point cell-level AUC gain at 0\,dB.
...
(iii) extending LRT and DP-TBD with KF back-ends for a full track-level
comparison against the ML pipelines.
