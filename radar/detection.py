...
from radar.inference import _ml_map, _gru_step_raw, _cnn_peaks, _gru_peaks
...
def _lrt_score(ppi: np.ndarray) -> np.ndarray:
    ...
    Normalises each sweep by its range-dependent noise floor then sums squared amplitudes.
    Returns a (n_az, n_range) score map.
    """
    noise_floor = np.percentile(ppi, 10, axis=(1, 2), keepdims=True).clip(1e-6)
...
def _dp_tbd_score(ppi: np.ndarray, max_vr: int = 3, max_vaz: int = 2) -> np.ndarray:
    ...
    Accumulates normalised amplitude along the best-scoring trajectory over all sweeps,
    allowing target motion of ±max_vr range bins and ±max_vaz azimuth bins per sweep.
...
def _roc_data(snr_db_val: float, n: int = 100, _v: int = 10):
    params = load_params()
    session = load_cnn_session()
    gru_session = load_gru_session()

    rng = np.random.default_rng(77)
    ml_sc, cfar_sc, gru_sc, lrt_sc, tbd_sc, lbs = [], [], [], [], [], []
